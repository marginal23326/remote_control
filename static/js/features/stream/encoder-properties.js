...
function setEncoderPropertyConstraints(constraints) {
    encoderPropertyConstraints = { ...constraints };
}

function setEncoderProperties(props) {
    ...
    for (const [key, value] of sortedEntries) {
        ...
        if (constraint) {
            if (constraint.value_type === "enum") {
                ...
                valHtml = `<input type="number" class="prop-val ${inputCls}" value="${escapeHtml(value)}"${constraint.min != null ? ' min="' + constraint.min + '"' : ""}${constraint.max != null ? ' max="' + constraint.max + '"' : ""}>`;
...
export { setEncoderPropertyConstraints, setEncoderProperties, readEncoderPropsFromDOM };
