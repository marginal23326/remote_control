...
class ClientAudioProcessor extends AudioWorkletProcessor {
    ...
    constructor(options: ClientAudioProcessorOptions) {
        super();
        ...
        for (let i = 0; i < inputChannelData.length; i++) {
            ...
            if (this.pointer >= this.bufferSize) {
                ...
                for (let j = 0; j < this.bufferSize; j++) {
                    ...
                    int16Chunk[j] = s < 0 ? s * 0x80_00 : s * 0x7fff;
                }
                this.port.postMessage({ pcmData: int16Chunk.buffer, type: "pcmData" }, [int16Chunk.buffer]);
                this.pointer = 0;
            }
        }
        return true;
...
class ServerAudioPlaybackProcessor extends AudioWorkletProcessor {
    ...
    constructor() {
        ...
        this.port.onmessage = (event: MessageEvent<ServerPcmMessage>) => {
    ...
    override process(_inputs: Float32Array[][], outputs: Float32Array[][]): boolean {
        ...
        for (let i = 0; i < length; i++) {
            ...
            if (this.isBuffering) {
                this.fadeVolume = Math.max(0, this.fadeVolume - this.fadeStep);
